...
if($_SESSION["CurrentGID_Code"] != 0) {
  ...
  $response = ["G_ID_code" => [], "d1" => [], "d1_side" => [],"d2" => [],"d3" => [], "currentColour" => [], "playerA" => [], "playerB" => [], "turn_id" => [], "closedWinner_UID" => []];
  ...
  if ($_GET["GridUpdate"] == "win") {
    $winUpdate = R::load("save", $viewingCode);
        ...
        if (count(array_unique($commonDiff_array)) == 1) {
          ...
          $winUpdate->winner_uid = json_encode([$_SESSION["id"], json_encode($checkGroup_array), rand(0,100), time()+16]);
...
function draw() {
  ...
  push()
      ...
      textAlign(LEFT)
      fill("#394a55")
      text("L#"+local_UID+":P#"+player_UID_1+"?"+player_UID_2+":G#"+Game_ID_code+":TU#"+turn_id_current+":WU#"+set_winner_UID+":§"+(((player_UID_1 == turn_id_current && turn_id_current == local_UID) || (player_UID_2 == turn_id_current && turn_id_current == local_UID)) && player_UID_2 > 0 && set_winner_UID == -1), width/85, height/50)
      pop()
      textAlign(CENTER)
    textSize(20);
  ...
  else { text("P2["+player_UID_2_usrn+"]", 0, 175); }
  pop()
