...
if($_SESSION["CurrentGID_Code"] != 0) {
  ...
  $fetch_response = ["G_ID_code" => [], "d1" => [], "d1_side" => [],"d2" => [],"d3" => [], "currentColour" => [], 
  "playerA" => [], "playerB" => [], "turn_id" => [], "closedWinner_UID" => [], "game_presetVal" => [], "total_movesSync" => []];
  $fetch_response["G_ID_code"] = $item->id;
  ...
  if (is_array($decodedD1)) { $fetch_response["d1"] = array_merge($fetch_response["d1"], $decodedD1); }
  if (is_array($decodedD1) && $item->game_mode != 2) { $fetch_response["d1_side"] = array_merge($fetch_response["d1_side"], $decodedD1_side); }
  else if ($item->game_mode == 2 ) { $fetch_response["d1_side"] = json_encode([]); }
  if (is_array($decodedD2)) { $fetch_response["d2"] = array_merge($fetch_response["d2"], $decodedD2); }
  if (is_array($decodedD3)) { $fetch_response["d3"] = array_merge($fetch_response["d3"], $decodedD3); }
  $fetch_response["currentColour"] = $item->turn_open; 
  $fetch_response["playerA"] = json_decode($item->uID_1);
  $fetch_response["playerB"] = json_decode($item->uID_2);
  $fetch_response["turn_id"] = json_decode($item->turn_id);
  $fetch_response["closedWinner_UID"] = json_decode($item->winner_uid);
  $fetch_response["game_presetVal"] = $item->game_mode;
  $fetch_response["total_movesSync"] = $_SESSION["local_totalMoves"];
...
// checking if clicked square can be placed in location and if it is a win or place square determied by client
// url types:
  ...
  if ($_GET["GridUpdate"] == "win") {
    $winUpdate = R::load("save", $viewingCode);
        ...
        if (count(array_unique($commonDiff_array)) == 1) {
          ...
          $winUpdate->winner_uid = json_encode([$_SESSION["id"], json_encode($checkGroup_array), rand(0,100), time()+13]);
...
if((isset($_GET["id"]) && $_GET["id"] == "loginCheck" )|| !isset($_SESSION["username"])){
    ...
    if($user == NULL){
    header("Location: login.php?id=error=1");
}
...
function checkData() {
  if (turn_id_current != local_UID) {
    fetch(window.location.href + '?check_data=1')
    .then(check_response => check_response.json())
    .then(data => { 
      if (client_totalMoves <= data.T_M || data.T_M == 0 ) {
        fetchData();
      }
    })
  }
  ...
  function fetchData() {
    ...
    .then(data => {
        ...
        //important for loading to not touch changing it stops game from working
...
function checklocation(base) {
  if (((player_UID_1 == turn_id_current && turn_id_current == local_UID) || (player_UID_2 == turn_id_current && turn_id_current == local_UID)) && player_UID_2 > 0 && set_winner_UID == -1) {
    ...
    if (check_lineX.length<4 && check_lineY.length<4 && check_lineND.length<4 && check_linePD.length<4) {location.href = "play.php?Locked="+base+"&&GridUpdate="+turn_colour_ghost;}
  }
}
...
pop()
  ...
  if (set_winner_UID != -1) {
    if (player_UID_1 != player_UID_2) { 
      if (player_UID_1 == set_winner_UID) { winning_username = player_UID_1 };
      if (player_UID_2 == set_winner_UID) { winning_username = player_UID_2 };
    }
    else {
...
echo "<div id=\"en0_splashDisplay\">Connect At: ".trim(shell_exec("ipconfig getifaddr en0"))."</div>";
  ...
  foreach($leaderboard_Aplicants as $row) {
    ...
    array_push($leaderB_statArray, $User_statpull->save3);
    }
  }
  ...
  for ($lb_display_counter = 0; $lb_display_counter<5; $lb_display_counter++) {
    if (isset(array_values($leaderB_globalArray)[$lb_display_counter])) {
      echo "<h2><span";if( $lb_display_counter==0 ){ echo " style=\"color:goldenrod\""; } if( $lb_display_counter==1 ){ echo " style=\"color:silver\""; } if( $lb_display_counter==2 ){ echo " style=\"color:#B87333\""; }
      echo ">#".($lb_display_counter+1).": ".array_keys($leaderB_globalArray)[$lb_display_counter]."</span> <span style=\"color:rgb(255, 183, 0)\">(".array_values($leaderB_globalArray)[$lb_display_counter]."👑)</span></h2>";
    }
  }
  echo "</div>";
