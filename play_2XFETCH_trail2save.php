...
if($_SESSION["CurrentGID_Code"] != 0) {
  ...
  $response = ["G_ID_code" => [], "d1" => [], "d1_side" => [],"d2" => [],"d3" => [], "currentColour" => [], 
  "playerA" => [], "playerB" => [], "turn_id" => [], "closedWinner_UID" => [], "game_presetVal" => []];
  ...
  if (is_array($decodedD1) && $item->game_mode != 2) { $response["d1_side"] = array_merge($response["d1_side"], $decodedD1_side); }
  else if ($item->game_mode == 2 ) { $response["d1_side"] = json_encode([]); }
  ...
  $response["game_presetVal"] = $item->game_mode;
}
else { $response = []; }
...
function draw() {
  ...
  push()
      ...
      fill("#394a55")
      textSize(14)
      text("L#"+local_UID+":P#"+player_UID_1+"?"+player_UID_2+":G#"+Game_ID_code+":T#"+turn_id_current+":WU#"+set_winner_UID+":§"+(((player_UID_1 == turn_id_current && turn_id_current == local_UID) || (player_UID_2 == turn_id_current && turn_id_current == local_UID)) && player_UID_2 > 0 && set_winner_UID == -1), width/85, height/50)
      pop()
      textAlign(CENTER)
    textSize(20);
